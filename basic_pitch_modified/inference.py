...
def get_audio_input_pyaudio(
    audio_original: np.ndarray, overlap_len: int, hop_size: int
) -> Tuple[Tensor, List[Dict[str, int]], int]:
    """
    Read wave file (as mono), pad appropriately, and return as
    windowed signal, with window length = AUDIO_N_SAMPLES
...
def run_inference_pyaudio(
    ...
    audio_windowed, _, audio_original_length = get_audio_input_pyaudio(audio_original, overlap_len, hop_size)
...
def predict_pyaudio(
    ...
    with no_tf_warnings():
        ...
        model_output = run_inference_pyaudio(audio_original, model, debug_file)
        min_note_len = int(np.round(minimum_note_length / 1000 * (AUDIO_SAMPLE_RATE / FFT_HOP)))
        midi_data, note_events = infer.model_output_to_notes(
            model_output,
            onset_thresh=onset_threshold,
